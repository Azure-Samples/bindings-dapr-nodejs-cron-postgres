...
async function processBatch(){
    ...
    fs.readFile(loc, 'utf8', (err, data) => {
        ...
        orders.forEach(order => {
            let sqlTableCmd = `create table if not exists orders (orderid INT, customer VARCHAR(50), price REAL);`;
            let sqlCmd = `insert into orders (orderid, customer, price) values (${order.orderid}, '${order.customer}', ${order.price});`;
            let tablePayload = `{"sql": "${sqlTableCmd}"} `;
            let payload = `{"sql": "${sqlCmd}"} `;
            console.log(payload);

            client.binding.send(postgresBindingName, "exec", "", JSON.parse(tablePayload));
